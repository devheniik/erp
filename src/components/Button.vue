<template>
    <button type="button" @click="handle()"
        :class="`inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-${type}-600 hover:bg-${type}-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-${type}-500`">
        {{label}}
    </button>
</template>
