...
<div class="flex" id="litepie-datepicker">
    ...
    <!-- @change=" $emit('change')" -->
    <litepie-datepicker as-single i18n="ru" :options="options" :disableDate="true" :placeholder="data.label" :formatter="formatter" :auto-apply="true" :modelValue="String(modelValue) ?? String(data.value_default)" @update:modelValue="$emit('update:modelValue', $event); $emit('change')"
    />
</div>
</template>
