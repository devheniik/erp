        ...
        <!-- <div class="hidden sm:block w-full"> -->
            <nav class="relative z-50 shadow bg-gray-100  flex justify-start divide-gray-200 w-full" aria-label="Tabs">
                <div v-for="(tab, tabIdx) in tabs" :key="tab.name" class="w-full " @click="clear(tabIdx, !tab.current)">
                    <a :class="[tab.current ? 'text-gray-900 bg-gray-200' : 'text-gray-800 hover:text-gray-900', 'mx-0.5 rounded-md h-6 flex items-center text-sm font-medium justify-center hover:bg-gray-200 focus:z-10']" :aria-current="tab.current ? 'page' : undefined">
                            <span>{{ tab.name }}</span>
                        </a>
