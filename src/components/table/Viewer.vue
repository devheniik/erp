        ...
        <div v-if="!isLoad && !isReload && data" class="w-full min-h-full flex flex-col justify-between">
            <div class="w-full h-full">
                ...
                <div class="w-full my-5 mx-2">
                    <component v-for="(component, i) in data.components?.start" :key="i" :data="component.data"
                        v-bind="component.config" :is="component.component"></component>
                </div>
                ...
                <div class="w-full flex flex-row items-center  ml-2.5 mt-3 justify-between">
                    ...
                    <div>
                        <button @click="filter_show = !filter_show" type="button"
                            class="inline-flex items-center  p-2 mr-2 border border-transparent rounded-full shadow-sm text-white bg-primary-600 hover:bg-primary-700">
                            <FilterIcon class="h-5 w-5" aria-hidden="true" />
                        </button>
                        <button @click="reLoad(); render_key++" type="button"
                            class="inline-flex items-center  p-2 mr-8 border border-transparent rounded-full shadow-sm text-white bg-danger-600 hover:bg-danger-700">
                ...
                <div class="my-5 mr-5">
                    <utable @sort="sort($event)"
                        :v-bind="{ row: data.show_row_select ?? false, select: data.show_row_burgers ?? false }"
                        :modalSelect=modalSelect @select="select($event)" :headers="data.headers"
                        v-model:body="data.data" :sort="data.sort"></utable>
                </div>

                <div class="w-full my-5 mx-2" v-if="data?.components?.end?.length">
                    <component v-for="(component, i) in data.components.end" :key="i" :data="component.data"
                        v-bind="component.config" :is="component.component"></component>
                </div>
            </div>

            <div class="w-full mb-1.5">
                <pagination v-if="data.meta?.pagination" @change="load" v-model:headers="data.headers"
                    :pagination="data.meta.pagination" v-model:page="data.page" v-model:limit="data.limit"></pagination>
            </div>
        </div>
