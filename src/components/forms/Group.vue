    ...
    <div>
        <div @click="isOpen = true">
              <div>
                ...
                <button v-for="(btn, i) in buttons" :key="i" alt="12313" @click="load(btn)" type="button" :class="[i + 1 == buttons.length ? 'rounded-r-md' : '', '-ml-px relative inline-flex items-center space-x-2 px-4 py-2 border border-gray-300 text-sm font-medium  text-gray-700 bg-gray-50 hover:bg-gray-100 focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500']">
                    <SearchIcon class="h-5 w-5 text-gray-400" aria-hidden="true" /> 
                </button>
                </div>
            </div> 
            <label :for="id" class="block text-sm font-medium text-gray-700">{{label}}</label> 
            <input :value="Number(modelValue)" :required="required" type="hidden" :name="id"  class="focus:ring-indigo-500 focus:border-indigo-500 block w-full rounded-none rounded-l-md pl-4 sm:text-sm border-gray-300"  />         
        </div>
