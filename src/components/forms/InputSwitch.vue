<template> 
      <SwitchGroup as="div" class="flex items-center">
            <SwitchLabel as="span" class="ml-3">
                <span class="text-sm font-medium text-gray-900">{{modelValue}}</span> 
            </SwitchLabel>
            <Switch :value="modelValue" @click="$emit('update:modelValue',$event.target.value)" :class="[modelValue ? 'bg-primary-600' : 'bg-gray-200', 'relative inline-flex flex-shrink-0 h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500']">
